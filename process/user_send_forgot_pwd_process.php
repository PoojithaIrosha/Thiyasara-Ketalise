...
if (empty($email)) {
    ...
    if ($user_rs->num_rows == 1) {
        ...
        MySQL::iud("UPDATE `user` SET `verification_code` = ? WHERE `email` = ?", [$unique_id, $email], 'ss');
        ...
        <div style="height:auto;padding:50px 50px 50px 50px;background-color:#edeff1"> <br>
            ...
            <div style="width:500px;height:auto;margin-top:0px;padding-bottom:80px;font-size:14px;background-color:white;text-align:center">
                ...
                <center>
                    ...
                    <div>
                        <a href="http://localhost/viva-wp/user_reset_password.php?uid=' . $unique_id . '" style="text-align:center;color:white;text-decoration:none; font-size: 16px; font-weight: bold; background-color:#2ec4b6; padding:15px 20px;" target="_blank">
                            Reset Your Password
                        </a>
                    </div>
                    <br>
